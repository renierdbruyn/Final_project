...
$req1 = mysql_query('select m1.id, m1.title, m1.timestamp, count(m2.id) as reps, users.id as userid, users.username from pm as m1, pm as m2,users where ((m1.user1="'.$obj->userdata['userid'].'" and m1.user1read="no" and users.id=m1.user2) or (m1.user2="'.$obj->userdata['userid'].'" and m1.user2read="no" and users.id=m1.user1)) and m1.id2="1" and m2.id=m1.id group by m1.id order by m1.id desc');
$req2 = mysql_query('select m1.id, m1.title, m1.timestamp, count(m2.id) as reps, users.id as userid, users.username from pm as m1, pm as m2,users where ((m1.user1="'.$obj->userdata['userid'].'" and m1.user1read="yes" and users.id=m1.user2) or (m1.user2="'.$obj->userdata['userid'].'" and m1.user2read="yes" and users.id=m1.user1)) and m1.id2="1" and m2.id=m1.id group by m1.id order by m1.id desc');
?>
This is the list of your messages:<br />
 
<a href="http://localhost/ci/index.php/welcome/newmsger" class="btn btn-large btn-primary disabled">New MSG</a><a href="http://localhost/ci/index.php/home1" id="left" class="btn btn-large btn-primary disabled">Go Home</a>
<!--<a href="newmsger" class="link_new_pm"></a><br />-->
